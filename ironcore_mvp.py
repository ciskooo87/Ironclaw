...
def run_evals(summary: Dict[str, Any], top_risks: List[Dict[str, Any]], eval_dir: Path, run_id: str | None = None, update_baseline: bool = False) -> Dict[str, Any]:
    ...
    if baseline:
        ...
        if metrics["critical_high"] > max(1, b.get("critical_high", 0) * 1.3):
            regressions.append({"metric": "critical_high", "baseline": b.get("critical_high"), "current": metrics["critical_high"], "severity": "warning"})
...
def render_markdown(summary: Dict[str, Any], top_risks: List[Dict[str, Any]], eval_result: Dict[str, Any]) -> str:
    ...
    if eval_result["regressions"]:
        lines.extend(["## Alertas de regressão", ""])
        for reg in eval_result["regressions"]:
            lines.append(f"- [{reg['severity']}] {reg['metric']}: baseline={reg['baseline']} atual={reg['current']}")
    return "\n".join(lines)
...
def run(
    ...
    logging.info("Dirs: input=%s processed=%s output=%s config=%s eval=%s", input_dir, processed_dir, output_dir, config_dir, eval_dir)
    ...
    eval_result = run_evals(summary, top_risks, eval_dir, run_id=run_id, update_baseline=update_baseline)
    ...
    logging.info("Pipeline concluído. Processados=%s Riscos=%s Issues=%s Eval=%s", summary["processed"], summary["risks"], summary["issues"], eval_result["status"])
    ...
    if fail_on_regression and eval_result["regressions"]:
        logging.error("Execução marcada como falha por regressões (--fail-on-regression).")
        return 4
    return 0
...
def main() -> None:
    ...
    parser.add_argument("--eval-dir", type=Path, default=DEFAULT_EVALS)
